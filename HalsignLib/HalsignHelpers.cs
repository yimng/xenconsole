    ...
    public static class HalsignHelpers
    {
        ...
        public static string GetFriendlyLicenseName(Host host)
        {
            if (MidnightRideOrGreater(host))
            {
                ...
                return (PropertyManager.GetFriendlyName("Label-host.edition-" + host.edition) ?? Messages.UNKNOWN);
            }
            if (host.license_params.ContainsKey("sku_type"))
            {
                string format = host.IsFloodgateOrLater() ? "Label-host.sku_type-FG-{0}" : "Label-host.sku_type-{0}";
                return PropertyManager.GetFriendlyName(string.Format(format, host.license_params["sku_type"].Replace(" ", "_").ToLowerInvariant()));
            }
            return Messages.UNKNOWN;
        }
        ...
        public static Dictionary<XenRef<VM>, string> GetVmHaRestartPrioritiesForApi(Dictionary<VM, VM.HA_Restart_Priority> settings)
        {
            ...
            {
                if ((((VM.HA_Restart_Priority) settings[vm]) != VM.HA_Restart_Priority.BestEffort) && (((VM.HA_Restart_Priority) settings[vm]) != VM.HA_Restart_Priority.DoNotRestart))
                {
                    dictionary[new XenRef<VM>(vm.opaque_ref)] = VM.PriorityToString(settings[vm]);
                }
            }
            return dictionary;
        }
        ...
        public static string StringFromMaxMinSize(long min, long max)
        {
            ...
            if (OrderOfMagnitudeDifference(min, max))
            {
        ...
        public static string StringFromMaxMinSizeList(List<long> minList, List<long> maxList)
        {
            ...
            {
                if ((minList[i] < 0L) && (maxList[i] < 0L))
                {
                    flag3 = true;
                }
                else if (minList[i] < 0L)
                {
                    max += maxList[i];
                    flag = true;
                }
                else if (maxList[i] < 0L)
                {
                    min += minList[i];
                    flag2 = true;
                }
            ...
            return StringFromMaxMinSize(min, max);
        }
        ...
        public static string StringFromMaxMinTimeList(List<long> minList, List<long> maxList)
        {
            ...
            return StringFromMaxMinTime(min, max);
        }
